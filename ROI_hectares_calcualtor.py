...
def lambda_handler(event, context):
    ...
    pol_geom = gpd.read_file(json.dumps(pol_shape))
    roi_geom = gpd.read_file(json.dumps(roi_shape))
    
    intersection_area = pol_geom.intersection(roi_geom,)
    
    intersection_area = intersection_area.loc[intersection_area!=None]
    
    if intersection_area.loc[intersection_area!=None].is_empty.all():
        return {
             "statusCode": 400,
             "body": "not intersecting geometries"
             }        
    
    intersection_area.to_file(path_to_tmp + "intersection.json", driver='GeoJSON')
    
    with open(path_to_tmp + "intersection.json", 'r') as f:
        json_file = json.load(f)
